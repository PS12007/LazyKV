...
The first run of this script found the reference disagreeing with itself.
scripts/01_kernel_repeatability.py isolates why (cuDNN decode is not repeatable for
identical inputs), and quality is now scored on lazykv.quality.QUALITY_STRATEGY, with the
fast kernel's floor kept as a reported comparison rather than hidden.
